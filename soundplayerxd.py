...
class SoundPlayer(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self._task_queue = asyncio.Queue(loop=self.bot.loop)
        self._thread_pool = ThreadPoolExecutor()
        self._cache_dir = os.path.join(self.bot.ROOT_DIR, 'cache')
        self._path_cache = LRUCache(20)
        self._volume = 1.0

        # Run task queue consumer
        asyncio.run_coroutine_threadsafe(self._play_queued(), loop=self.bot.loop)
    ...
    async def play(self, ctx, source: str, *, times: str = '1'):
        ...
        task = functools.partial(self._play_track, voice, track)
    ...
    def _play_track(self, voice, track):
        audio_source = discord.PCMVolumeTransformer(discord.FFmpegPCMAudio(track.source, options='-loglevel quiet'))
        audio_source.volume = self._volume
        voice.play(audio_source)

    async def _create_track(self, source):
        if not await self._is_link(source):
            source = await self._create_path(source)
        else:
            source = await self._download_link(source)
        return _Track(source) if source is not None else None
    ...
    async def _create_path(self, audio):
        ...
        for path, _, files in os.walk(self._CLIPS_DIR):
            for ending in PLAYER_FILE_TYPES:
                if f'{audio}{ending}'.lower() in [f.lower() for f in files]:
